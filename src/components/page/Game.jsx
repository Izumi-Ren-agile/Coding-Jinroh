...
export const Game = (props) => {
  ...
  useEffect(() => {
    if (
      ...
      gameObject.startingTurn + gameObject.codingMaxTime >
      ...
      gameObject.startingTurn + gameObject.meetingmaxTime >
        Math.floor(Date.now() / 1000)
    ) {
  ...
  const handleRunCode = async () => {
    ...
    const plusCode = code !== "" ? code : gameObject.editorHistory[gameObject.editorHistory.length-1].code;
    console.log("足し合わせるコード", plusCode);
    const adjustedCode =
      "import java.util.*; import java.io.*; import java.lang.*; public class Main{" +
      gameObject.main +
      plusCode +
      "}";
      ...
      {gameObject.property ? (
        ...
        <Contents>
          <Content70>
            ...
            <Tag secondText={""} colorMode={gameObject.gamePhase}>
              実行結果{" "}
              <Tooltip
                title="実行ボタンを押すと実行結果が表示されます。入力例に対して、期待される出力を満たしていると、正誤判定が〇になります。"
                placement="top"
              >
                <QuestionCircleOutlined />
              </Tooltip>
          ...
          <Project
            question={gameObject.questionText.replace(/\\n/g, "\n")}
            secondText={""}
            gameObject={gameObject}
          />
        </Contents>
